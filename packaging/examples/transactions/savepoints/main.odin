...
main :: proc() {
	...
	fmt.println("")
	fmt.println("== outer transaction with savepoint ==")
	...
	// This outer transaction change should survive.
	err, ok = sqlite.db_exec(
		db,
		"UPDATE accounts SET balance_cents = balance_cents + 500 WHERE id = 1",
	)
	if !ok {
		fmt.println("outer transaction update failed:", sqlite.error_string(err))
		_, _ = sqlite.db_rollback(db)
		return
	}

	print_balance(db, "after outer update before savepoint", 1)
	print_balance(db, "after outer update before savepoint", 2)

	err, ok = sqlite.db_savepoint(db, "transfer_attempt")
	if !ok {
		fmt.println("savepoint failed:", sqlite.error_string(err))
		_, _ = sqlite.db_rollback(db)
		return
	}

	// These savepoint-local changes will be rolled back.
	err, ok = sqlite.db_exec(
		db,
		"UPDATE accounts SET balance_cents = balance_cents - 300 WHERE id = 1",
	)
	if !ok {
		fmt.println("savepoint debit failed:", sqlite.error_string(err))
		_, _ = sqlite.db_rollback(db)
		return
	}
	...
	err, ok = sqlite.db_exec(
		db,
		"UPDATE accounts SET balance_cents = balance_cents + 300 WHERE id = 2",
	)
	if !ok {
		fmt.println("savepoint credit failed:", sqlite.error_string(err))
		_, _ = sqlite.db_rollback(db)
		return
	}

	print_balance(db, "inside savepoint before rollback_to", 1)
	print_balance(db, "inside savepoint before rollback_to", 2)

	err, ok = sqlite.db_rollback_to(db, "transfer_attempt")
	if !ok {
		fmt.println("rollback_to savepoint failed:", sqlite.error_string(err))
		_, _ = sqlite.db_rollback(db)
		return
	}

	// After rollback_to, release the savepoint so the outer transaction can continue cleanly.
	err, ok = sqlite.db_release(db, "transfer_attempt")
	if !ok {
		fmt.println("release savepoint failed:", sqlite.error_string(err))
		_, _ = sqlite.db_rollback(db)
		return
	}

	print_balance(db, "after rollback_to savepoint", 1)
	print_balance(db, "after rollback_to savepoint", 2)
	...
	fmt.println("")
	fmt.println("== final balances after outer commit ==")
	print_balance(db, "final", 1)
	print_balance(db, "final", 2)

	fmt.println("")
	fmt.println("savepoint example completed successfully")
}